...
public class IniciarApiController implements IniciarApi {
    ...
    public ResponseEntity<JsonApiBodyResponseSuccess> iniciarPost(@ApiParam(value = "body" ,required=true )  @Valid @RequestBody JsonApiBodyRequest body) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<JsonApiBodyResponseSuccess>(objectMapper.readValue("{  \"id\" : \"id\",  \"rol\" : \"rol\",  \"token\" : \"token\"}", JsonApiBodyResponseSuccess.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
